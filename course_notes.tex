...
many of them!), and it happens that within 32bits floats the closest float to
	$2.5e8$ is $2.5e8 + 16$ (note that $5104$ is a multiple of 16). If you take $N = 100$ instead, you'll start
...
\lstinputlisting[style=C]{data/hash_tables.c}
...
(with no root). Nodes in the tree that have no children are called leaves.
\end{definition}
...
tree contains $N = \#(V)$ nodes, then its height $H$ can be as large as $N$
...
in a totally ordered set and the following BST condition holds : 
	\begin{enumerate}
...
In particular, keys must be distinct. Note that it is possible to relax this last
limitation, and/or to allow pre-ordered set of keys, by replacing strict
inequalities above by non strict ones. This requires additional constraints though, 
so that searches in the tree do not end up unpractical.
below).  

\medskip

With the above definition, finding a key in a BST is as simple as :

\begin{itemize}
	\item If the tree is empty then the search fails.
	\item Else if the tree is non empty and the key of the root is equal to 
		the searched key the search is successful.
	\item Else if the root has a key which is smaller than the searched key, then
		proceed with the search in its right sub-tree
	\item Else proceed with the search in its left sub-tree.
\end{itemize}

Insertions are very similar. 
The following elementary implementation is based on recursion. The values
here are assumed to be integers for simplicity. In the tutorials, you will 
deal with a situation where the tree nodes are contained in the data 
(this is called an intrusive tree), 
...
but searches and insert always pick the same side in case of equality.
...
/* Note : In this implementation duplicate vals are allowed */
...
deleted only has one (or zero) children : in that case it suffices to delete
...
permutation}\right\},$$ equipped with uniform probability, where $\sigma(i) =
j$ means that the key with value $i$ is the $j^{th}$ one to be inserted.
.
...
\begin{definition}
	...
	a key and the following heap property holds :
	\begin{enumerate}
		\item $\forall v \in V$, $\forall w \in lst(v)$, $w.key <=
			v.key$.
		\item $\forall v \in V$, $\forall w \in rst(v)$, $w.key <=
...
not be unique). Heaps are typically used for implementing {\it priority-queues}, where extraction
...
corresponding to the highest depth, which must only be full from the left.
...
in order to reset it. The correct governing rule for the {\it sift-down} is to swap
...
complete tree is that that {\bf such trees can easily be implemented as arrays}, thus
...
%TODO : Heap sort
%%%%%%%%%%%%%%%%%%
Another interesting and efficient application of heaps that we discussed in
class, in addition to the already mentioned priority-queues, is an in place 
and $O(\log(N))$ sorting algorithm: {\it heap sort}.
